...
public:
	 ...
	 */
	explicit LuaObject( LuaState* luastate ); //28

	LuaObject( const LuaObject&& ); //31

	~LuaObject(); //33
	
	LuaObject& operator=( const LuaObject&& ); //35

	/** 
	 * Calls method defined in Lua script.
	 * @param name Name of the Lua method to be called.
	 */
	void call( const char* name ); //36
	
	int resume(const char* name); //210 | Not on iOS.
...
private:
	void	registerClosure( const char* name, const LuaFunctor* lf, LuaState::CFunction dispatcher ); //401
