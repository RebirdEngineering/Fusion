...
public:
	...
	/**  */
	void			setRow3( int i, const float3& c )								{assert(i>=0&&i<ROWS); m[i][0]=c[0]; m[i][1]=c[1]; m[i][2]=c[2]; m[i][3]=c[3];}
	 ...
	 * 
	 */
	void			setReflection( const float4&  )									{} //?
	...
	/**  */
	const float3&	getRow3( int i ) const											{return *reinterpret_cast<const float3*>(m[i]);}
	...
	/** Returns determinant of 3x4 top left sub-matrix. */
