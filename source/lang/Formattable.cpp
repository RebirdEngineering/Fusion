...
static void parseNumberPattern( 
	const std::string& pattern, int pos,
...
Formattable::Formattable( const std::string& value )
{
	m_type = VALUE_STRING;
	m_str = value;
}
...
{
	assert( m_type == VALUE_DOUBLE ); //Assert match (private KA3D) or line 96 of RCSDEBUG
	return m_dbl;
}

std::string Formattable::stringValue() const
{
	assert( m_type == VALUE_STRING ); //Assert match (private KA3D) or line 102 of RCSDEBUG
	return m_str;
}

int	Formattable::format( char* buffer, int size, const std::string& pattern, int pos ) const
{
	...
	else if ( VALUE_STRING == m_type )
	{
		...
		if ( count > size ) 
			count = size;
		memcpy(buffer, m_str.data(), count);
		if ( needed < size )
			buffer[needed] = 0;
		else if ( size > 0 )
			buffer[size-1] = 0;
