...
{
	assert( (back-front) > FLT_MIN ); //Assert on line 56
...
float4x4 float4x4::inverse() const
{
	...
	assert( det > FLT_MIN || det < -FLT_MIN ); // singular [NOTE] Assert on line 293
...
{
	assert( axis.length() > FLT_MIN ); // ensure that Axis isn't null vector [NOTE] Assetr on line 358
...
{
	assert( &v != v1 ); //Assert on line 385
	float* d = &v1->x;
...
{
	assert( &v != v1 ); //Assert on line 394
	float* d = &v1->x;
...
{
	assert( &v != v1 ); //Assert on line 404
	float* d = &v1->x;
