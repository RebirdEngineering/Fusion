...
{
	const double RAND_SCALE = 1.0f / (double(RAND_MAX)+1.0f);
	return double(rand()) * RAND_SCALE;
}

double RandomUtil::getRandom(double begin, double end )
{
...
{
	assert( r1 >= 0.f ); //Assert on line 112
	assert( r2 >= 0.f ); //Assert on line 113
